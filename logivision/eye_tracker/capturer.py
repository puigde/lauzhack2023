    ...
    def detect_eyes(
        ...
        for (x, y, w, h) in coords:
            ...
            if int(face_img.shape[0] * 0.1) < eye_center < int(face_img.shape[1] * 0.4):
                ...
                int(face_img.shape[0] * 0.5) < eye_center < int(face_img.shape[1] * 0.9)
            ):
    ...
    def process(self, frame: numpy.ndarray, l_threshold, r_threshold):
        ...
        try:
            ...
            if left_eye is not None:
                left_key_points = self.blob_track(
                    left_eye, l_threshold, self.previous_left_blob_area
                )
            ...
            if right_eye is not None:
                right_key_points = self.blob_track(
                    right_eye, r_threshold, self.previous_right_blob_area
                )
